...
library TickMath {
    ...
    //checked
    function nextWordLow (int24 tick, bool buySide) internal pure returns (int24 newTick){
        //overflow may be possibile
        if (buySide){
            newTick = ((tick + 0x100) >> 8) << 8;
        } else {
            newTick = (((tick)>>8)<<8) - 1;
        }
    }
    ...
    //checked
    function setSingle (uint256 p, uint8 pos, bool setToOne) internal pure returns(uint256 q)  {
        if (setToOne){
            q = p | (1 << uint256(pos));
        } else {
            q = p & (0 << uint256(pos));
    ...
    //checked
    function setRange (uint256 p, uint8 fromPos, uint8 toPos, bool setToOne) public pure returns (uint256 q) {
        //fromPos < toPos
        uint256 mask = (2 ** (toPos - fromPos + 1) - 1) << fromPos;
        q = p | mask;
        if (!setToOne) {
            q -= mask;
        } 
